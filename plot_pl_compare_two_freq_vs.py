...
parser.add_argument(\
    ...
    default='pl_compare.png', help='plot file name')        
parser.add_argument(\
...
for i, rx_type in enumerate(cfg.rx_types):
    ...
    for iplot in range(nplot):
            ...
            if iplot == 0:
                ax[0].scatter(pl_omni_plot[iplot][I],pl2_omni_plot[iplot][I],s=9, c=colors[iplot])
                ax[0].set_title('Real Data')   
                ax[0].set_xlabel('2.3GHz Path loss(dB)')
                ax[0].set_ylabel('28GHz Path loss(dB)')
                ax[0].set_xlim(70,160)
                ax[0].set_ylim(95,180)
                ax[0].grid()
            else:
                ax[1].scatter(pl_omni_plot[iplot][I],pl2_omni_plot[iplot][I],s=9, c=colors[iplot])
                ax[1].set_title('Train Model')   
                ax[1].set_xlabel('2.3GHz Path loss(dB)')
                ax[1].set_xlim(70,160)
                ax[1].set_ylim(95,180)
                # ax[1].set_ylabel('28GHz Path loss(dB)')
                ax[1].grid()
