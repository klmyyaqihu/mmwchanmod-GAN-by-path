...
class ChanMod(object):
    ...
    def transform_ang(self, dvec, fspl_ls, nlos_ang, nlos_pl_ls, fit=False):
        ...
        if fit:
            ...
            # el_path = os.path.join(model_dir, self.el_preproc_fn)
            ...
            # aod_theta_standard = self.aod_el_scaler.transform(aod_theta_rel)
            # aoa_theta_standard = self.aod_el_scaler.transform(aoa_theta_rel)
            Xang = np.hstack(\
                (aoa_phi_rel/180, aoa_theta_rel/180,\
                 aod_phi_rel/180, aod_theta_rel/180))
        
        # aod_theta_standard = (aod_theta_rel-np.mean(aod_theta_rel))/np.std(aod_theta_rel)
        # aoa_theta_standard = (aoa_theta_rel-np.mean(aoa_theta_rel))/np.std(aoa_theta_rel)       
    ...
    def sample_path(self, dvec, fspl_ls, rx_type, link_state=None, return_dict=False):
        ...
        # for i in range(nlink):
        #     nlos_pl[i,:] = np.tile(fspl1[i]+self.max_ex_pl, self.npaths_max).astype(np.float32)
        #     nlos_pl2[i,:] = np.tile(fspl2[i]+self.max_ex_pl, self.npaths_max).astype(np.float32)
        ...
        # los_pl2  = np.zeros((nlink,), dtype=np.float32)
        ...
        # data['nlos_pl2'] = nlos_pl2
        # data['los_pl'] = los_pl
        # data['los_pl2'] = los_pl2
